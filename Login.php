...
?>
<?php
$dbConnectionPro = parse_ini_file("Project.ini");
extract($dbConnectionPro);
$conn = new PDO($dsn, $user, $password);
    ...
    if(isset($_POST["send"]))
    {
        ...
        {
            //log in right
            $sqlName ="SELECT name, password from user where UserId =:code Limit 1;";
            ...
            if($executeN)
            {
                ...
                if (password_verify($pp, $executeN['password']))
                {
                    ...
                    $_SESSION["logId"]=$id;
                    $_SESSION["logpp"]=$pp;
                    header("Location: Index.php");  //back to last page
                    exit( );               
                }
                else {
